...
Hard filters (pre-search) are applied via Qdrant payload filters using `filter_*` params:
`filter_tags` (`key` or `key=value`), `filter_evidence`, `filter_category`, `filter_scope`,
`filter_artifact_type`, `filter_artifact_ref`, `filter_entity`, `filter_source`,
`filter_access_entity`, `filter_app`. `filter_mode` controls tag/evidence matching (`all` or `any`).
`filter_app` filters by the MCP client that wrote the memory (e.g., `claude-code`, `cursor`).
